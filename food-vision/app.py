...
if selected == "Home":
    ...
    if pred_button:
        ...
        st.write(df[['Top 5 Predictions','Scores']].sort_values('Scores', ascending= False))
        plotPrediction(df['Top 5 Predictions'].to_numpy(),df['Scores'].to_numpy())
...
if selected == "Projects":
    ...
    if model_selected == '10 Classes':
        ...
        st.header("Evaluate the model : ")
        history_load=np.load('./history/history_10.npy',allow_pickle='TRUE').item()
        fig = plotHistory(history_load)
        st.write(fig)
    if model_selected == '101 Classes (final model)':
        ...
        st.header("Evaluate the model : ")
        history_load=np.load('./history/history_101.npy',allow_pickle='TRUE').item()
        fig = plotHistory(history_load)
        st.write(fig)
